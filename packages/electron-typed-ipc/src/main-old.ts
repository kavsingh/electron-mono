...
type ElectronTypedIpcMain<TSchema extends Schema<Definition>> = Readonly<{
	[TChannel in keyof TSchema]: TSchema[TChannel] extends OperationWithChannel<Query>
		? {
				handleQuery: (
					handler: (
						...
						| TSchema[TChannel]["response"]
						| Promise<TSchema[TChannel]["response"]>,
				) => RemoveHandlerFn;
			}
		: TSchema[TChannel] extends OperationWithChannel<Mutation>
			...
			: TSchema[TChannel] extends OperationWithChannel<SendFromMain>
				? {
						send: (
							payload: keyof TSchema[TChannel]["payload"] extends never
								? undefined
